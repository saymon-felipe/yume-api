...
router.post("/:post_id/comment", login, (req, res, next) => {
    _postsService.commentInPost(req.usuario.id, req.params.post_id, req.body.comment).then(() => {
        let response = functions.createResponse("Comentário feito com sucesso", null, "POST", 200);
        return res.status(200).send(response);
    }).catch((error) => {
        return res.status(500).send(error);
    })
...
router.get("/:post_id/return_comments", login, (req, res, next) => {
    _postsService.returnPostComments(req.params.post_id).then((results) => {
        let response = functions.createResponse("Retorno dos comentários do post", results, "GET", 200);
        return res.status(200).send(response);
    }).catch((error) => {
        return res.status(500).send(error);
    })
...
router.post("/:post_id/exclude", login, (req, res, next) => {
    _postsService.excludePost(req.params.post_id, req.usuario.id).then(() => {
        let response = functions.createResponse("Post excluído com sucesso", null, "POST", 200);
        return res.status(200).send(response);
    }).catch((error) => {
        return res.status(500).send(error);
    })
...
router.post("/:post_id/edit", login, (req, res, next) => {
    _postsService.editPost(req.params.post_id, req.usuario.id, req.body.post_text).then(() => {
        let response = functions.createResponse("Post excluído com sucesso", null, "POST", 200);
        return res.status(200).send(response);
    }).catch((error) => {
        return res.status(500).send(error);
...
router.get("/return_friends_feed", login, (req, res, next) => {
    const offset = req.query.offset || 0;
    const limite = req.query.limite || 20;

    _postsService.returnFeed(req.usuario.id, offset, limite, true).then((results) => {
        let response = functions.createResponse("Retorno do feed", results, "GET", 200);
        return res.status(200).send(response);
    }).catch((error) => {
        return res.status(500).send(error);
